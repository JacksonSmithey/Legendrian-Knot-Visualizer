...
def main():
    ...
    f = (4-x**2+x)*e1 -1/3*e1**3 + (4-x**2)*e2 - 1/5*e2**5 +e2**2 - (
            (4-x**2+x)*e3 -1/3*e3**3 + (4-x**2)*e4 - 1/5*e4**5 +e4**2)
    ...
    #+ (1/256)*x**6*e1 - (1/8)*x**3*e2*e1 + e2**2*e1 - 0.5*e1
    #- (9/8)*x*e2 - (9/128)*x**4 + (81/256)*x**2 + (1/256)*x**6
    #+ (1/8)*x**3*e2 + e2**2 - 0.5)
    #"""
    ...
    for i in np.arange(xmin, xmax, RES):
        NEW_DFE = []
        for dfe in DFE:
            NEW_DFE.append(sp.simplify(sp.Subs(dfe, x, i)))
            ...
            match num_e:
                case 1:
                    if("I" in str(value[0]) or "I" in str(value[1])):
                        ...
                        #plot point
                        ax.scatter(xi, y, z, c='black')
                        front_ax.scatter(xi, z, c='black')
                        top_ax.scatter(xi, y, c='black')
